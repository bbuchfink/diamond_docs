...
the task to be run by the program.
  ...
  - `test`
    ...
    Run a series of test cases and verify the hashes of the output files
    against reference values. This command will exit with code `0` if all
...
## Alignment options
  ...
  - `--sensitive`
    ...
    Enable sensitive mode designed for full sensitivity for hits >40% identity.
    Without using this option, the default (fast) mode will run which is designed
    for hits of >70% identity and short read aligment.
...
## Memory & performance options
  ...
  - `--block-size/-b #`  
    ...
    The default value is `-b2.0`. The parameter can be increased for
    better performance but values of >20 are not recommended.
